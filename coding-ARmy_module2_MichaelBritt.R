...
pseed.sum.max%>%
  ggplot(aes(x=amp.sum.mean,y=met.mean,col=fish))+geom_point(size=2)+geom_errorbar(aes(ymin=amp.sum.mean-amp.sum.se, ymax=amp.sum.mean+amp.sum.se),width=0.05,size=0.5,color="black")+geom_smooth(method="lm")+theme_classic()
