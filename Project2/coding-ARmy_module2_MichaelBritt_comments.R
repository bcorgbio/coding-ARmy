...
#CPK: It's generally not a good idea to share code that will install something. Best to comment this out (as I did).
library(tidyverse)
library(features)
library(ggplot2)
...
#CPK: This (VVV) wouldn't run b/c of an error in the "find.peaks" function, specifically "f$speaks" should have been "f$peaks". [-2] So this table in the next few lines had zero rows (i.e., no data). Function redefined so it will run:
...
#CPK: It would have been much more concise to define your se function early and calculated se with mean. [-1]. No need to merge a new table, just  . . 

pseed.sum.max <- pseed.max %>%
  group_by(fish,bl.s)%>% # grouped by fish and speed
  summarize(amp.sum.mean=mean(amp.sum),amp.sum.se=standard_error(amp.sum))
