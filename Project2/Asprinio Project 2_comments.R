...
#CPK: ^^^ Writing functions should be concise, too. Could have been as simpley as....
...
#CPK: It would have been much more concise to define your se function early and calculated se with mean. [-1] Like  . . .

pseed.sum.max <- pseed.max %>%
  group_by(fish,speed)%>% # grouped by fish and speed
  summarize(amp.sum.mean=mean(amp.sum),amp.sum.se=calc.se(amp.sum))
...
# CPK: When we say plot "something 1 vs something 2" we generally be y vs x, so in this case, met rate (on the y) vs amp.sum (on the x). We don't need error bars in this case, but if we did, not sure why you would want them to be black.Don't you want them colored like the rest of the data so you know to which fish they belong? I.e,

pseed.sum.max %>%
  ggplot(aes(x=amp.sum.mean,y=amp.met.rate,col=fish))+geom_point()+geom_smooth(method="lm")+theme_classic()
